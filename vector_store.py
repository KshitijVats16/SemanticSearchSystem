...
class VectorStore:
    ...
    def add(
        ...
        logger.info(
            f"Added {len(metadata_list)} vectors. Total vectors: {len(self.metadata)}"
        )
    ...
    def search(
        ...
        if self._embeddings.shape[0] == 0:
            logger.warning("Search requested on empty vector store.")
            return []
    ...
    def save(self) -> None:
        ...
        logger.info(
            f"Saved {len(self.metadata)} vectors to {self.store_dir}"
        )

    def load(self) -> bool:
        if not self._emb_path.exists() or not self._meta_path.exists():
            logger.warning(
                f"Vector store not found in {self.store_dir}"
            )
            return False
        ...
        logger.info(
            f"Loaded {len(self.metadata)} vectors from {self.store_dir}"
        )
